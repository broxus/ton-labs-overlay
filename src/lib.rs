use adnl::{
    common::{
        add_object_to_map, add_object_to_map_with_update, deserialize_bundle, get256, hash,
        hash_boxed, serialize, serialize_append, AdnlPeers, KeyId, KeyOption, Query, QueryResult,
        Subscriber, UpdatedAt, Version,
    },
...
use ton_api::{
    ton::{
        ...
        catchain::{
            blockupdate::BlockUpdate as CatchainBlockUpdate, FirstBlock as CatchainFirstBlock,
            Update as CatchainBlockUpdateBoxed,
        },
        fec::{type_::RaptorQ as FecTypeRaptorQ, Type as FecType},
        overlay::{
            broadcast::{
                id::Id as BroadcastOrdId, tosign::ToSign as BroadcastToSign,
                Broadcast as BroadcastOrd, BroadcastFec,
            },
            broadcast_fec::{id::Id as BroadcastFecId, partid::PartId as BroadcastFecPartId},
            message::Message as OverlayMessage,
            node::{tosign::ToSign as NodeToSign, Node},
            nodes::Nodes,
...
impl OverlayShard {
    ...
    fn calc_broadcast_id(&self, data: &[u8]) -> Result<Option<BroadcastId>> {
        ...
        let added = add_object_to_map(&self.owned_broadcasts, bcast_id.clone(), || {
    ...
    fn create_fec_recv_transfer(
        ...
        tokio::spawn(async move {
            loop {
                ...
                match overlay_shard_wait.owned_broadcasts.get(&bcast_id_wait) {
                    Some(transfer) => {
                        if let OwnedBroadcast::RecvFec(transfer) = transfer.val() {
    ...
    async fn receive_fec_broadcast(
        ...
        let stats = if let Some(stats) = overlay_shard.stats_per_transfer.get(bcast_id) {
            ...
            add_object_to_map(&overlay_shard.stats_per_transfer, bcast_id.clone(), || {
                Ok(TransferStats {
        ...
        let transfer = loop {
            ...
            if !add_object_to_map(&overlay_shard.owned_broadcasts, bcast_id.clone(), || {
            ...
            let ok = match overlay_shard
                .owned_broadcasts
                .insert(bcast_id.clone(), transfer)
            {
                Some(removed) => match removed.val() {
                    OwnedBroadcast::WillBeRecvFec => true,
                    _ => false,
                },
                _ => false,
            };
        ...
        if !transfer
            .history
            .update(bcast.seqno as u64, TARGET_BROADCAST)
            .await?
        {
            return Ok(());
        }
        ...
        stats
            .val()
            .resent
            .fetch_add(neighbours.len() as u64, Ordering::Relaxed);
        // Transit broadcasts will be traced untagged
        overlay_shard
            .distribute_broadcast(
                raw_data,
    ...
    fn update_stats(&self, dst: &Arc<KeyId>, tag: u32, is_send: bool) -> Result<()> {
        ...
        let stats = if let Some(stats) = stats.get(dst) {
            ...
            add_object_to_map(stats, dst.clone(), || Ok(lockfree::map::Map::new()))?;
        ...
        let stats = if let Some(stats) = stats.val().get(&tag) {
            ...
            add_object_to_map(stats.val(), tag, || Ok(AtomicU64::new(0)))?;
...
impl OverlayNode {
    ...
    pub fn add_consumer(
        ...
        add_object_to_map(&self.consumers, overlay_id.clone(), || Ok(consumer.clone()))
    }
    ...
    pub fn add_public_peer(
        ...
        add_object_to_map_with_update(&shard.nodes, ret.clone(), |old_node| {
    ...
    pub fn delete_public_peer(
        ...
        match shard
            .bad_peers
            .insert_with(peer.clone(), |_, prev| prev.is_none())
        {
    ...
    fn add_overlay(
        ...
        let added = add_object_to_map(&self.shards, overlay_id.clone(), || {
        ...
        if added {
            let shard = self
                .shards
                .get(overlay_id)
                .ok_or_else(|| error!("Cannot add overlay {}", overlay_id))?;
...
impl Subscriber for OverlayNode {
    async fn try_consume_custom(&self, data: &[u8], peers: &AdnlPeers) -> Result<bool> {
        ...
        #[cfg(feature = "telemetry")]
        {
            ...
            overlay_shard
                .val()
                .update_stats(peers.other(), tag.0, false)?;
        }
        if bundle.len() == 2 {
            // Private overlay
            ...
            match bundle.remove(0).downcast::<Broadcast>() {
                Ok(Broadcast::Overlay_BroadcastFec(bcast)) => {
                    OverlayShard::receive_fec_broadcast(overlay_shard.val(), bcast, data, &peers)
                        .await?;
                    Ok(true)
                }
                Ok(Broadcast::Overlay_Broadcast(bcast)) => {
                    OverlayShard::receive_broadcast(overlay_shard.val(), bcast, data, &peers)
                        .await?;
                    Ok(true)
                }
